...
export default function UploadPage() {
  ...
  const handleAIAnalyze = () => {
    ...
    for (let i = 8; i < duration; i += 8) {
      const obj = objects[i%objects.length];
      const startTime = lastTime;
      const endTime = i;
      generatedTimestamps.push({ time: `${startTime} to ${endTime} sec`, location: obj.name, description: obj.desc });
      lastTime = i;
    }
  ...
  return (
    <div className="bg-gray-50 flex flex-col p-8 gap-8 w-full">
      
      {/* Flex container for Video + Timestamp (Keeps them Side-by-Side) */}
      ...
      {videoUrl && (
        <>
            ...
            {/* Map Upload Section */}
            <h2 className="text-3xl font-extrabold text-gray-800 text-center">Map</h2>
            <MapUploadSection />
        </>
        )}
    </div>
